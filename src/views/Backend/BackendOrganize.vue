...
const organizeDataOld = [
  {
    ...
    organize_datetime: '112/01/02 13:22:51',
    staff_no: 1
  }
]

const organizeData = reactive({ ...organizeDataOld[0] })

// footer 儲存按鈕
const saveData = () => {
  // 更新 homeData 的時間戳
  organizeData.organize_datetime = getCurrentDateTime()
...
// footer 取消按鈕
const resetData = () => {
  Object.assign(organizeData, organizeDataOld[0])
}
...
// 計算文字 & 標題的當前字數
const titleLength = computed(() => organizeData.organize_title.length)
const titleEnLength = computed(() => organizeData.organize_title_en.length)
const trendTxtLength = computed(() => organizeData.trend_txt.length)
const trendTxtEnLength = computed(() => organizeData.trend_txt_en.length)
...
<template>
  <div class="backend-block-flex">
    <!-- 大標題 -->
    <div class="backend-page-title">公司組織圖</div>
    ...
    <div class="backend-block-flex">
      <label
        ...
        <input
          type="text"
          maxlength="20"
          v-model="organizeData.organize_title"
          class="input-backend"
          placeholder="標題"
        />
      </label>
      ...
      <label
        ...
        <input
          type="text"
          maxlength="255"
          v-model="organizeData.organize_title_en"
          class="input-backend"
          placeholder="標題_英文"
        />
      </label>
      ...
      <div class="input-backend-pic-flex">
        ...
        <div>
          <input
            type="file"
            @change="(event) => handleFileUpload(event, null, organizeData, 'organize_title_pic')"
            class="input-backend"
          />
        </div>
        ...
        <div class="w-full cursor-pointer" v-show="showBanner">
          <img
            :src="organizeData.organize_title_pic"
            alt="image Preview"
            class="pic"
            @click="() => openImageInNewWindow(organizeData.organize_title_pic)"
          />
        </div>
      </div>
    </div>
    ...
    <div class="backend-block-flex">
      <!-- * 標題 & 看圖片按鈕 -->
      <div class="backend-block-title">成長趨勢</div>
    </div>

    <label
      >文字
      <span class="label-note">* 最多 255 字 ({{ trendTxtLength }} / 255)</span>
      <textarea
        v-autogrow
        type="text"
        maxlength="255"
        v-model="organizeData.trend_txt"
        class="input-backend"
        placeholder="區塊1文字1"
      ></textarea>
    </label>
    ...
    <label
      ...
      <textarea
        ...
        type="text"
        maxlength="2000"
        v-model="organizeData.trend_txt_en"
        class="input-backend"
    ...
    <div class="backend-footer">
      <div class="flex gap-2">
        最後更新 :
        <div v-text="organizeData.organize_datetime"></div>
        <div v-text="organizeData.staff_no"></div>
      </div>
