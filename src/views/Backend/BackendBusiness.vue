...
const businessDataOld = [
  {
    ...
    business_datetime: '112/01/02 13:22:51',
    staff_no: 1
  }
]

const businessData = reactive({ ...businessDataOld[0] })

// footer 儲存按鈕
const saveData = () => {
  // 更新 homeData 的時間戳
  businessData.business_datetime = getCurrentDateTime()
...
// footer 取消按鈕
const resetData = () => {
  Object.assign(businessData, businessDataOld[0])
}
...
// 計算文字 & 標題的當前字數
const titleLength = computed(() => businessData.business_title.length)
const titleEnLength = computed(() => businessData.business_title_en.length)
const trendTxtLength = computed(() => businessData.business_txt.length)
const trendTxtEnLength = computed(() => businessData.business_txt_en.length)
...
<template>
  <div class="backend-block-flex">
    <!-- 大標題 -->
    <div class="backend-page-title">營業實績</div>
    ...
    <div class="backend-block-flex">
      <label
        ...
        <input
          type="text"
          maxlength="10"
          v-model="businessData.business_title"
          class="input-backend"
          placeholder="標題"
        />
      </label>
      ...
      <label
        ...
        <input
          type="text"
          maxlength="255"
          v-model="businessData.business_title_en"
          class="input-backend"
          placeholder="標題_英文"
        />
      </label>
      ...
      <div class="input-backend-pic-flex">
        ...
        <div>
          <input
            type="file"
            @change="(event) => handleFileUpload(event, null, businessData, 'business_title_pic')"
            class="input-backend"
          />
        </div>
        ...
        <div class="w-full cursor-pointer" v-show="showBanner">
          <img
            :src="businessData.business_title_pic"
            alt="image Preview"
            class="pic"
            @click="() => openImageInNewWindow(businessData.business_title_pic)"
          />
        </div>
      </div>
    </div>
    ...
    <div class="backend-block-flex">
      <!-- * 標題 & 看圖片按鈕 -->
      <div class="backend-block-title">營業實績</div>
    </div>

    <label
      >文字
      <span class="label-note">* 最多 1000 字 ({{ trendTxtLength }} / 1000)</span>
      <textarea
        v-autogrow
        type="text"
        maxlength="1000"
        v-model="businessData.business_txt"
        class="input-backend"
        placeholder="區塊1文字1"
      ></textarea>
    </label>
    ...
    <label
      >文字_英文
      <span class="label-note">* 最多 2000 字 ({{ trendTxtEnLength }} / 2000)</span>
      <textarea
        v-autogrow
        type="text"
        maxlength="2000"
        v-model="businessData.business_txt_en"
        class="input-backend"
        placeholder="區塊1文字1_英文"
      ></textarea>
    </label>
    ...
    <div class="backend-footer">
      <div class="flex gap-2">
        最後更新 :
        <div v-text="businessData.business_datetime"></div>
        <div v-text="businessData.staff_no"></div>
      </div>
